...
module.exports = function (context) {
    ...
    var targetDir  = path.join(projectRoot, "platforms", "android", "src","main","java", "com.qdc.plugins.baidu".replace(/\./g, path.sep));
    ...
    if (['after_plugin_add', 'after_plugin_install', 'after_platform_add'].indexOf(context.hook) === -1) {
        ...
        // sync the content
        fs.readFile(path.join(context.opts.plugin.dir, 'src', 'android', 'BaiduPushReceiver.java'), {encoding: 'utf-8'}, function (err, data) {
            if (err) {
                throw err;
            }
